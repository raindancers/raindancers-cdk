...
def scan(input_bucket, input_key, download_path, definitions_path, tmp_path):
    ...
    try:
        ...
        if scan_summary.returncode == 0:
            ...
            status = INFECTED
            if os.environ["SEND_SECURITY_HUB_EVENT"] in os.environ:
                send_security_hub_finding(
                    input_bucket,
                    input_key,
                    message,
                )
...
def send_security_hub_finding(input_bucket, input_key, message):

    finding_time =datetime.datetime.now(datetime.timezone.utc).isoformat()
    finding_id = hashlib.sha256(message.endcode()).hexdigest()
    finding = []
    product_arn = f'arn:aws:securityhub:{os.environ["AWS_REGION"]}:{os.environ["ACCOUNTID"]}:product/{os.environ["ACCOUNTID"]}/default'
    finding.append({
        'AwsAccountId': os.environ['ACCOUNTID'],
        'CreatedAt': finding_time,
        'Description': (message[:1020] + '...') if len(message) > 1023 else message,
        'GeneratorId': 'serverless-clamscan',
        'Id': finding_id,
        ...
        'Title': 'Malware / Virus Detected',
        'UpdatedAt': finding_time,
    })
